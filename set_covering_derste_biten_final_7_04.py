    ...
    for k in range(k_range[0], k_range[1] + 1):
        ...
        for label in range(k):
            ...
            if route_time > TIME_LIMIT and len(cluster_nodes) > 1:
                # If the cluster's route time exceeds the limit, split the cluster into two subclusters using k-means
...
def compute_cost_details(final_clusters, coordinates):
    ...
    overall_cost = 0.0
    for cluster in final_clusters:
        # For cost details, we can still use the NN-based tsp_distance.
        # Alternatively, you could also use tsp_distance_elkai if you prefer.
...
def write_final_clusters_csv(final_clusters, coordinates, part_number):
    ...
    # For convenience, convert each nested list (cluster) to a string
...
if __name__ == "__main__":
    # List of input CSV file paths (replace these with your actual CSV filenames)
    input_csv_files = [
