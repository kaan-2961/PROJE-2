...
class ClusterManager:
    def __init__(self, coordinates: List[List[float]], k: Union[int, Tuple[int, int]]):
        """
        Initializes the ClusterManager.
        
        Args:
            coordinates: A list of [latitude, longitude] lists.
            k: A tuple (min_k, max_k) specifying the range of the number of clusters
               to try for the range-based K-means method.
        """
        if not isinstance(k, tuple):
            raise ValueError("For range-based K-means, 'k' must be a tuple (min_k, max_k).")
        # Convert nested lists to tuples internally
        self.coordinates = [tuple(coord) for coord in coordinates]
        self.k = k
        self.clusters: Dict[str, Dict[str, List[Tuple[float, float]]]] = {}
    ...
    def run_clustering(self, method_name, **kwargs):
        ...
        if 'k_range' not in kwargs and isinstance(self.k, tuple):
            kwargs['k_range'] = self.k

        clusters = clustering_function(self.coordinates, **kwargs)
        if isinstance(clusters, dict):
            self.clusters[method_name] = {cluster_id: [tuple(c) for c in points] for cluster_id, points in clusters.items()}
        else:
            self.clusters[method_name] = {f"{method_name}_cluster_{i+1}": [tuple(c) for c in cluster] for i, cluster in enumerate(clusters)}

    def calculate_route_time(self, cluster_points: List[Tuple[float, float]], speed_km_per_hr, service_time_hr):
        """Calculate the total time for a given cluster (using TSP optimization)."""
        if not cluster_points:
            return 0.0
            
        # Flatten any nested coordinate structures
        flat_points = []
            ...
            if isinstance(point, tuple) and len(point) == 2:
                ...
                for coord in point:
                    if isinstance(coord, (list, tuple)) and len(coord) == 2:
                        flat_points.append(tuple(coord))
                        
        if not flat_points:
            return 0.0
            
        depot = flat_points[0]
        return calculate_cluster_time_with_tsp(flat_points, depot, speed_km_per_hr, service_time_hr)

    def calculate_route_time_nn(self, route: List[Tuple[float, float]], speed_km_per_hr: float, service_time_hr: float) -> float:
        """Calculate the total time for a route as constructed by nearest neighbor."""
        total_distance = 0.0
        ...
        total_service_time = (len(route) - 1) * service_time_hr
        return travel_time + total_service_time

    def get_all_clusters_with_methods(self, speed_km_per_hr, service_time_hr) -> List[Dict[str, any]]:
        """Retrieve all clusters with their methods, IDs, points, and costs."""
        all_clusters = []
        for method_name, clusters in self.clusters.items():
            for cluster_id, cluster_points in clusters.items():
                cost_function = self.calculate_route_time if method_name.startswith('kmeans_range') else self.calculate_route_time_nn
                cost = cost_function(cluster_points, speed_km_per_hr, service_time_hr)
                if method_name == 'randomized_nearest_neighbor':
                    cost += random.uniform(-0.01, 0.01)
                all_clusters.append({
                    "method": method_name,
...
def calculate_cluster_time_with_tsp(cluster: List[Tuple[float, float]], depot: Tuple[float, float], speed_km_per_hr: float, service_time_hr: float) -> float:
    """Calculate total time for a cluster using TSP optimization."""
    try:
        ...
        for point in cluster:
            if isinstance(point, (list, tuple)) and len(point) == 2:
                normalized_cluster.append((float(point[0]), float(point[1])))
            else:
                raise TypeError(f"Invalid coordinate format: {point}")

        # Create distance matrix
        distance_matrix = np.zeros((num_points, num_points))
        for i in range(num_points):
            for j in range(num_points):
                distance_matrix[i, j] = haversine(normalized_cluster[i], normalized_cluster[j])

        if num_points == 2:
            travel_distance = distance_matrix[0, 1] * 2  # Round trip
            return (travel_distance / speed_km_per_hr) + service_time_hr
        ...
        return (total_distance / speed_km_per_hr) + ((num_points - 1) * service_time_hr)
    except Exception as e:
        print(f"TSP calculation error: {e}")
        return float('inf')
...
def nearest_neighbor_clustering(coords: List[Tuple[float, float]], speed_km_per_hr: float, service_time_hr: float, tmax: float) -> List[List[Tuple[float, float]]]:
    """Cluster points using nearest neighbor approach with time constraint."""
    depot = coords[0]
    remaining_coords = coords[1:]
    n = len(remaining_coords)
    clusters = []
    visited = [False] * n

    while not all(visited):
        # Start with random unvisited point
        start_index = random.choice([i for i, v in enumerate(visited) if not v])
        current_cluster = [depot, remaining_coords[start_index]]
        visited[start_index] = True
        last_point = remaining_coords[start_index]

        while True:
            # Find nearest unvisited neighbor
            nearest_index, min_dist = -1, float('inf')
            for i in range(n):
                if not visited[i]:
                    dist = haversine(last_point, remaining_coords[i])
                    if dist < min_dist:
                        min_dist = dist
                        nearest_index = i
            ...
            # Check if adding this point would exceed time constraint
            potential_cluster = current_cluster + [remaining_coords[nearest_index]] + [depot]
            total_time = calculate_cluster_time_with_tsp(potential_cluster, depot, speed_km_per_hr, service_time_hr)

            if total_time <= tmax:
                current_cluster.append(remaining_coords[nearest_index])
                visited[nearest_index] = True
                last_point = remaining_coords[nearest_index]
            else:
                break

        clusters.append(current_cluster + [depot])
...
def randomized_nearest_neighbor_clustering(coords: List[Tuple[float, float]], speed_km_per_hr: float, service_time_hr: float, tmax: float, k: int = 3) -> List[List[Tuple[float, float]]]:
    """Cluster points using randomized nearest neighbor approach."""
    depot = coords[0]
    remaining_coords = coords[1:]
    n = len(remaining_coords)
    clusters = []
    ...
    while not all(visited):
        ...
        while True:
            # Find k nearest unvisited neighbors
            distances = []
            for i in range(n):
                if not visited[i]:
                    dist = haversine(last_point, remaining_coords[i])
            ...
            distances.sort()
            k_nearest = [i for _, i in distances[:min(k, len(distances))]]
            chosen_index = random.choice(k_nearest)

            # Check time constraint
            potential_cluster = current_cluster + [remaining_coords[chosen_index]] + [depot]
            total_time = calculate_cluster_time_with_tsp(potential_cluster, depot, speed_km_per_hr, service_time_hr)

            if total_time <= tmax:
                current_cluster.append(remaining_coords[chosen_index])
                visited[chosen_index] = True
                last_point = remaining_coords[chosen_index]
            else:
                break
...
def kmeans_clustering_with_range(
    ...
    for k in range(min_k, max_k + 1):
        ...
        initial_clusters = [coords[labels == i].tolist() for i in range(k)]
        final_clusters = {}

        for i, cluster_customers in enumerate(initial_clusters):
            if not cluster_customers:
                continue
                
            reclustered = _recluster_if_needed([tuple(c) for c in cluster_customers], depot, speed_km_per_hr, service_time_hr, tmax)
            if reclustered:
                final_clusters[f"cluster_{i+1}"] = [[depot] + c + [depot] for c in reclustered]

        if final_clusters:
            all_results[f"k_{k}"] = [item for sublist in final_clusters.values() for item in sublist]
...
def _recluster_if_needed(
    ...
    """Recursively recluster if time constraint is violated."""
    if not customers:
        return []
        
    tour = [depot] + customers + [depot]
    total_time = calculate_cluster_time_with_tsp(tour, depot, speed, service)
    
    if total_time <= tmax or len(customers) <= 2:
        return [customers]
    ...
    for label in (0, 1):
        ...
        if subset:
            re_reclustered = _recluster_if_needed(subset, depot, speed, service, tmax)
            if re_reclustered:
                clusters.extend(re_reclustered)
        elif len(customers) == 1:
            clusters.append([customers[0]])

    return clusters

def enforce_unique_assignment(
    tours_with_method: List[Dict[str, any]],
    depot: Tuple[float, float],
    speed_km_per_hr: float,
    service_time_hr: float,
    tmax: float
) -> List[Dict[str, any]]:
    """Ensure each point appears in exactly one tour."""
    point_to_original_cluster = {}
    for tour_data in tours_with_method:
        for point in tour_data['points'][1:-1]:
            if tuple(point) not in point_to_original_cluster:
                point_to_original_cluster[tuple(point)] = tour_data

    final_clusters = []
    assigned_points = set()

    for tour_data in tours_with_method:
        unique_tour_points = [tour_data['points'][0]]  # Start with depot
        for point in tour_data['points'][1:-1]:
            if tuple(point) not in assigned_points:
                unique_tour_points.append(point)
                assigned_points.add(tuple(point))

        if len(unique_tour_points) > 1:
            unique_tour_points.append(tour_data['points'][-1])  # End with depot
            cost_function = ClusterManager.calculate_route_time if tour_data['method'].startswith('kmeans_range') else ClusterManager.calculate_route_time_nn
            cost = cost_function(None, unique_tour_points, speed_km_per_hr, service_time_hr)
            
            final_clusters.append({
                "method": tour_data['method'],
                "cluster_id": f"unique_{len(final_clusters) + 1}",
                "points": unique_tour_points,
                "cost": cost
            })
    ...
    for cluster in final_clusters:
        key = tuple(sorted(tuple(p) for p in cluster['points'][1:-1]))
        if key not in seen:
            seen.add(key)
...
def optimal_set_covering(universe, sets):
    """Solve the set covering problem to select optimal clusters."""
    ...
    # Constraints: Cover all points
    point_indices = {tuple(point): idx for idx, point in enumerate(universe)}
    for point in universe:
        covering_sets = [set_vars[i] for i, s in enumerate(sets) 
                        if tuple(point) in [tuple(p) for p in s['points'][1:-1]]]
        solver.Add(solver.Sum(covering_sets) >= 1)

    status = solver.Solve()

    if status == pywraplp.Solver.OPTIMAL:
        return [sets[i] for i in range(len(sets)) if set_vars[i].solution_value() == 1]
    else:
...
cluster_manager = ClusterManager(coordinates, k=(10, 11))

cluster_manager.add_clustering_method(
    "nearest_neighbor", 
    lambda coords, **kwargs: nearest_neighbor_clustering(coords, speed_km_per_hr, service_time_hr, tmax)
)

cluster_manager.add_clustering_method(
    "randomized_nearest_neighbor", 
    lambda coords, **kwargs: randomized_nearest_neighbor_clustering(coords, speed_km_per_hr, service_time_hr, tmax, k=3)
)

cluster_manager.add_clustering_method(
    "kmeans_range", 
    lambda coords, **kwargs: kmeans_clustering_with_range(
        coords[1:], coords[0], speed_km_per_hr, service_time_hr, tmax, 
        k_range=kwargs.get('k_range', (10, 12)))
)

cluster_manager.run_clustering("nearest_neighbor")
cluster_manager.run_clustering("randomized_nearest_neighbor")
cluster_manager.run_clustering("kmeans_range")

# Get all clusters and solve set covering
all_clusters = cluster_manager.get_all_clusters_with_methods(speed_km_per_hr, service_time_hr)
universe = [tuple(coord) for coord in coordinates[1:]]
optimal_clusters = optimal_set_covering(universe, all_clusters)

# Final processing
if optimal_clusters:
    depot = tuple(coordinates[0])
    final_clusters = enforce_unique_assignment(optimal_clusters, depot, speed_km_per_hr, service_time_hr, tmax)
    
    print("\n--- Final Optimal Clusters ---")
    for cluster in final_clusters:
        print(f"Method: {cluster['method']}, Cost: {cluster['cost']:.2f} hours")
        print(f"Route: {cluster['points']}\n")
else:
