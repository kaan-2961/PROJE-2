...
def tsp_solver(coordinates, customer_indices):
    """
    A simple TSP solver (nearest neighbor heuristic).
...
def kmeans_clusters(coordinates, k_range):
    ...
    for k in range(k_range[0], k_range[1]+1):
        ...
        for label in range(k):
            ...
            route_time = tsp_solver(coordinates, cluster_nodes)
            if route_time > TIME_LIMIT and len(cluster_nodes) > 1:
                ...
                for sub_label in [0, 1]:
                    ...
                    sub_route_time = tsp_solver(coordinates, sub_cluster_nodes)
            ...
            for clust in clust_list:
                if clust != best and node in clust.nodes:
                    ...
                    if clust.method.startswith("kmeans"):
                        clust.route_time = tsp_solver(coordinates, clust.nodes)
                    else:
...
def get_final_cluster_coordinates(final_clusters, coordinates):
    ...
    for cluster in final_clusters:
        cluster_coords = [coordinates[node] for node in cluster.nodes]
        nested.append(cluster_coords)
    return nested

# -----------------------------
# Plotting functions (with option to save images)
# -----------------------------
...
def write_final_clusters_csv(final_clusters, coordinates, part_number):
    """Write a CSV file (set_covering_osm_data_partX.csv) containing the nested list of final clusters (as coordinates)."""
    nested_list = get_final_cluster_coordinates(final_clusters, coordinates)
    # For convenience, we convert each nested list (cluster) to a string
    df = pd.DataFrame({'Final Cluster Coordinates': [str(cluster) for cluster in nested_list]})
    csv_filename = f"split_merge_osm_data_part{part_number}.csv"
...
def process_input_file(input_csv, part_number, k_range):
    """
    For one input CSV file:
      - Read coordinates from CSV (assume two columns: lat, lon).
      - Run clustering and set covering.
      - Plot input coordinates and final clusters (save images).
      - Compute cost details.
      - Write a CSV file with final clusters nested list.
      - Return a summary dictionary for Excel.
    """
    # Read input CSV file
    df_in = pd.read_csv(input_csv)
    # Assume the CSV has two columns: latitude and longitude
    ...
    # Get nested list and save final clusters plot
    nested_list = get_final_cluster_coordinates(final_clusters, coordinates)
    ...
    # Compute cost details and overall cost
    cost_df, overall_cost = compute_cost_details(final_clusters, coordinates)
    # Write final clusters CSV file
    final_csv_file = write_final_clusters_csv(final_clusters, coordinates, part_number)
    
    # Prepare summary information for Excel
    summary = {
...
# -----------------------------
# Main processing: iterate over multiple input CSV files and write one Excel summary file.
# -----------------------------
...
if __name__ == "__main__":
    # List of input CSV file paths (assume you have as many as needed, e.g., 12)
    # For demonstration, replace these with your actual CSV filenames.
    input_csv_files = [
    ...
    # You can adjust the k_range as desired.
