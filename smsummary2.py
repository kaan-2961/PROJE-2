...
class Cluster:
    def __init__(self, data_points, cluster_id=None, color=None):
        ...
        self.attempts_left = 4  # You can modify this if needed
        if color is None:
...
def explore_split_branches(current_clusters, sub_clusters, speed_km_per_hr, service_time_hr, tmax, history_log):
    ...
    def recursive_merge(clusters_to_merge, remaining_sub_clusters):
        ...
        # Branch 2: Keep the first sub-cluster separate
...
def plot_input_coordinates(coordinates, filename=None):
    # coordinates is expected to be a list of [lat, lon] pairs.
    try:
        lats = [float(coord[0]) for coord in coordinates]
        lons = [float(coord[1]) for coord in coordinates]
    except (IndexError, ValueError) as e:
        print("Error processing coordinates:", e)
        return
    plt.figure(figsize=(8, 6))
    plt.scatter(lons, lats, color='blue', s=50, label='Input Points')
...
def plot_final_clusters(clusters, filename=None):
    plt.figure(figsize=(8, 6))
    for cluster in clusters:
        pts = cluster.data_points
        lats = pts[:, 0]
        lons = pts[:, 1]
        plt.scatter(lons, lats, color=cluster.color, s=50, label=cluster.id)
        # Plot centroid with a black 'x'
        plt.plot(cluster.centroid[1], cluster.centroid[0], marker='x', color='black', markersize=10)
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
...
def compute_cost_details(final_clusters):
    ...
    for cluster in final_clusters:
        ...
        data.append({
            ...
            'Centroid Lat': round(cluster.centroid[0], 4),
            'Centroid Lon': round(cluster.centroid[1], 4)
        })
...
# -----------------------------
# PROCESSING SINGLE INPUT CSV FILE (SPLIT–MERGE APPROACH) WITH TWO ITERATIONS
# -----------------------------
def process_input_file_split_merge(input_csv, part_number):
    # Read the CSV file. The first cell is expected to be the header "Final Cluster Coordinates"
    df_in = pd.read_csv(input_csv)
    if df_in.columns[0].strip() == "Final Cluster Coordinates":
        coordinate_strings = df_in["Final Cluster Coordinates"].tolist()
        # Each cell is a string representation of a nested list of coordinate pairs.
        coordinates = [ast.literal_eval(cell) for cell in coordinate_strings if isinstance(cell, str)]
    else:
        coordinates = df_in.values.tolist()
    
    # Instead of taking only the first cell, combine all rows:
    if not coordinates:
        print(f"No valid coordinates found in {input_csv}.")
        return {}
    
    # For the input plot, we combine all points from all clusters.
    combined_coords = []
    for group in coordinates:
        combined_coords.extend(group)
    input_plot_file = f"input_coordinates_part{part_number}.png"
    plot_input_coordinates(combined_coords, filename=input_plot_file)
    
    # Use all groups from the CSV as the input groups.
    ...
    for run_number in range(FIRST_RUN_NUM_RUNS):
        ...
        print(f"\n=== FIRST RUN {run_number + 1} for file {input_csv} ===")
    ...
    first_run_plot_file = f"first_run_clusters_part{part_number}.png"
    if best_run_clusters_first:
        plot_final_clusters(best_run_clusters_first, filename=first_run_plot_file)
        cost_df_first, overall_cost_first, overall_time_first = compute_cost_details(best_run_clusters_first)
    else:
        cost_df_first = pd.DataFrame()
        overall_cost_first = 0
        overall_time_first = 0

    # ----- SECOND RUN -----
    if best_run_clusters_first:
        final_cluster_points = [cluster.data_points.tolist() for cluster in best_run_clusters_first]
        print(f"\n--- Starting Second Run for file {input_csv} using first run best clusters ---")
        
        # Update parameters for second run
        global max_merge_attempts_per_cluster, max_iterations
        orig_merge_attempts = max_merge_attempts_per_cluster
        orig_iterations = max_iterations
        ...
        for run_number in range(SECOND_RUN_NUM_RUNS):
            ...
            print(f"\n=== SECOND RUN {run_number + 1} for file {input_csv} ===")
        ...
        second_run_plot_file = f"second_run_clusters_part{part_number}.png"
        if best_run_clusters_second:
            plot_final_clusters(best_run_clusters_second, filename=second_run_plot_file)
            ...
            second_run_plot_file = ""
        
        max_merge_attempts_per_cluster = orig_merge_attempts
    ...
    summary = {
        ...
        'first_run': {
            ...
            'overall_cost': overall_cost_first,
            'overall_time': overall_time_first,
            'cost_df': cost_df_first,
            'clusters_plot_file': first_run_plot_file,
            'all_runs_initial_metrics': all_runs_initial_metrics_first,
            'best_run_metrics': best_run_metrics_first
        },
        'second_run': {
            'num_final_clusters': best_run_metrics_second[0] if best_run_metrics_second else 0,
            'overall_cost': overall_cost_second,
            'overall_time': overall_time_second,
            'cost_df': cost_df_second,
            'clusters_plot_file': second_run_plot_file,
            'all_runs_initial_metrics': all_runs_initial_metrics_second if best_run_clusters_second else [],
            'best_run_metrics': best_run_metrics_second
        },
    ...
    with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
        for summary in summaries:
            ...
            overall_info = pd.DataFrame({
                ...
                'First Run - Final Clusters': [summary['first_run']['num_final_clusters']],
                'First Run - Overall Cost ($)': [summary['first_run']['overall_cost']],
                'First Run - Overall Time (hrs)': [summary['first_run']['overall_time']],
                'Second Run - Final Clusters': [summary['second_run']['num_final_clusters']],
                'Second Run - Overall Cost ($)': [summary['second_run']['overall_cost']],
                'Second Run - Overall Time (hrs)': [summary['second_run']['overall_time']]
            })
            overall_info.to_excel(writer, sheet_name=sheet_name, startrow=0, index=False)
            summary['first_run']['cost_df'].to_excel(writer, sheet_name=sheet_name, startrow=5, index=False)
            start_row_second = 5 + len(summary['first_run']['cost_df'].index) + 3
            summary['second_run']['cost_df'].to_excel(writer, sheet_name=sheet_name, startrow=start_row_second, index=False)
            ...
            worksheet.write('H12', f"First Run Clusters Plot (Part {summary['part_number']})")
            worksheet.insert_image('H13', summary['first_run']['clusters_plot_file'], {'x_scale': 0.5, 'y_scale': 0.5})
            if summary['second_run']['clusters_plot_file']:
                worksheet.write('A40', f"Second Run Clusters Plot (Part {summary['part_number']})")
                worksheet.insert_image('A41', summary['second_run']['clusters_plot_file'], {'x_scale': 0.5, 'y_scale': 0.5})
            worksheet.write('H40', f"Final Clusters CSV File (Part {summary['part_number']})")
